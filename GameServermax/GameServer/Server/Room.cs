...
namespace GameServer
{
	public class Room
	{
		...
		public Room (NetServer server, int Pattern)
		{
			_server = server;
			GamePattern = Pattern;
			if (GamePattern == PatternCode.Multiple) { _maxCount = 2; }
			else if (GamePattern == PatternCode.quartic) { _maxCount = 4; }
			else if (GamePattern == PatternCode.hexgon) { _maxCount = 6; }
